...
import com.project.negocio.connection.FabricaConexao;

public class ContatoDao {

	private final String INSERT = "INSERT INTO CONTATO (NOME, TELEFONE, EMAIL, RUA, NUMERO, VALOR, DESCONTO) VALUES (?,?,?,?,?,?,?)";
	private final String UPDATE = "UPDATE CONTATO SET NOME=?, TELEFONE=?, EMAIL=?, RUA=?, NUMERO=?, VALOR=?, DESCONTO=? WHERE ID=?";
	...
	public void inserir(Contato contato) {
		if (contato != null) {
			...
			try {
				...
				pstm.setInt(5, contato.getNumero());
				pstm.setDouble(6, contato.getValor());
				pstm.setDouble(7, contato.getDesconto());
				pstm.execute();
	...
	public void atualizar(Contato contato) {
		if (contato != null) {
			...
			try {
				...
				pstm.setString(5, contato.getRua());
				pstm.setInt(6, contato.getNumero());
				pstm.setDouble(7, contato.getValor());
				pstm.setDouble(8, contato.getDesconto());
				pstm.execute();
				JOptionPane.showMessageDialog(null, "Contato alterado com sucesso");
				FabricaConexao.fechaConexao(conn);

			} catch (Exception e) {
				JOptionPane.showMessageDialog(null, "Erro ao atualizar contato no banco de dados " + e.getMessage());
			}
		} else {
	...
	public List<Contato> getContatos() {
		...
		try {
			...
			while (rs.next()) {
				...
				contato.setDesconto(rs.getDouble("desconto"));
				contato.setValortotal(rs.getDouble("valortotal"));
